...
const Category = () => {
    ...
    const handleSearch = async (event) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/cate/search?q=${query}`);
    ...
    return (
        <div className="overflow-x-auto">
            ...
            {
                edit && <CatUpdatePost id={edit} onClose={hanldleEditClose} onUpdate={fetchData} />
            }
            {
                remove && <CatDeletePost id={remove} onClose={handleDeleteClose} onDelete={fetchData} />
            }
