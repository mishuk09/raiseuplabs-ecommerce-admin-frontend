...
const NewArrival = () => {
    ...
    const handleSearch = async (event) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/new/search?q=${query}`);
    ...
    return (
        <div className="overflow-x-auto">
            ...
            {
                add && <NewAddPost onClose={handleAddClose} onAdd={fetchData} />
            }
            {
                edit && <NewUpdatePost id={edit} onClose={hanldleEditClose} onUpdate={fetchData} />
            }
            {
                remove && <NewDeletePost id={remove} onClose={handleDeleteClose} onDelete={fetchData} />
            }
